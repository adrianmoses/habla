...
def _patched_app(db_reachable: bool = True) -> Iterator[TestClient]:
    ...
    with (
        ...
        # Test settings have no real cloud keys, use the placeholder DB DSN, and
        # configure no La Libreta integration; the #016/#017/#033 startup
        # fail-fasts are covered directly in test_session_auth /
        # test_deploy_hardening / test_handoff_startup, so no-op them here.
        patch("api.main.require_cloud_secrets", lambda _cfg: None),
        patch("api.main.require_secure_db_credentials", lambda _cfg: None),
        patch("api.main.require_integration_config", lambda _cfg: None),
