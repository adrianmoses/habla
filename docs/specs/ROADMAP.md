...
| 009 | Deployment: delete the llama.cpp GPU compose service and drop all `nvidia` GPU reservations — app container becomes CPU-only (Postgres + AGE `db` service unchanged); also removes `cuda_bootstrap.py` and loads `.env` via python-dotenv | in-progress | [009-gpu-free-deploy](009-gpu-free-deploy/spec.md) |
...
| 011 | Remove on-device-only tooling: `download_model.py`, `benchmark_*` scripts, the `finetune/` package, and the fine-tune notebook (`cuda_bootstrap.py` removed in #009) | planned | — |
